...
/* LAB6: YOUR CODE */
#define BIG_STRIDE  (2147483647) /* you should give a value, and is ??? */
...
stride_dequeue(struct run_queue *rq, struct proc_struct *proc) {
      ...
      */
    if(proc == NULL || rq == NULL) return;
    if(rq->lab6_run_pool == NULL) return;
    rq->lab6_run_pool = skew_heap_remove(rq->lab6_run_pool,
                                         &(proc->lab6_run_pool),
                                         proc_stride_comp_f);
    proc->rq = NULL;
    rq->proc_num -= 1;
    skew_heap_init(&(proc->lab6_run_pool)); // IMPORTANT
}
/*
